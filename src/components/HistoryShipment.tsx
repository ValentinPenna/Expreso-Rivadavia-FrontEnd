...
const HistoryShipment: React.FC<IHistoryShipmentProps> = ({ orders }) => {
  ...
  return (
    <div className="w-full my-4 pt-10 flex flex-col gap-2">
      <h1 className="text-primary text-2xl lg:text-4xl font-bold text-center">Ordenes</h1>
      {orders.map((order: Orders) => (
        <div key={order.id} className="bg-white shadow-lg p-6 rounded-lg relative">
          <div className="flex items-center cursor-pointer" onClick={() => handleToggleExpandedOrder(order.id)}>
            <svg
            ...
            <p className="md:text-xl text-md font-bold mx-4">{`Orden: ${new Date(order.date).toLocaleDateString()}`}</p>
          ...
          {expandedOrder.includes(order.id) && (
            <div className="ml-8 mt-2 md:text-xl text-sm">
              <p>
                Estado de la orden:
              ...
              <p className='flex flex-row'>
                Id del envío:<span className='text-primary ml-2'>{order.id.substring(0, 5)}...</span>
                <button
