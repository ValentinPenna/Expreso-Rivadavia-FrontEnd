    ...
    return (
        <>
            ...
            {showedOrders?.map((order) => (
                <div key={order.id} className="flex flex-row bg-white justify-between p-2 md:p-4 lg:p-6 shadow-lg rounded-lg">
                    <h3 className="font-bold text-xs md:text-base w-1/4 text-start">{new Date(order.date).toLocaleDateString()}</h3>
                    <h3 className="font-bold text-xs md:text-base w-1/4 text-center">{order.user.lastName}, {order.user.name}</h3>
                    <h3 className="font-bold text-xs md:text-base w-1/4 text-center">{order.shipments.locality_origin.name} / {order.shipments.locality_destination.name}</h3>
                    <h3 className="font-bold text-xs md:text-base w-1/4 text-end">{order.status}</h3>
                </div>
            ))
            }
        </>
